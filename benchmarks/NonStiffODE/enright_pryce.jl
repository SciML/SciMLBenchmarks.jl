...
sa1prob = ODEProblem{false}(sa1sys, y[1:4] .=> 1.0, (0, 20.0), dt = 1.0e-2; u0_constructor = x -> SVector(x...))
...
sa2prob = ODEProblem{false}(sa2sys, y[1:9] .=> 0.0, (0, 120.0), dt = 5.0e-4; u0_constructor = x -> SVector(x...))

sa3eqs = [
    D(y[1]) ~ -1.0e4 * y[1] + 100 * y[2] - 10 * y[3] + y[4],
    D(y[2]) ~ -1.0e3 * y[2] + 10 * y[3] - 10 * y[4],
    D(y[3]) ~ -y[3] + 10 * y[4],
    D(y[4]) ~ -0.1 * y[4],
]
@mtkcompile sa3sys = ODESystem(sa3eqs, t)

sa3prob = ODEProblem{false}(sa3sys, y[1:4] .=> 1.0, (0, 20.0), dt = 1.0e-5; u0_constructor = x -> SVector(x...))

sa4eqs = [D(y[i]) ~ -i^5 * y[i] for i in 1:10]
@named sa4sys_raw = ODESystem(sa4eqs, t)
sa4sys = structural_simplify(sa4sys_raw)

sa4prob = ODEProblem{false}(sa4sys, y[1:10] .=> 1.0, (0, 1.0), dt = 1.0e-5; u0_constructor = x -> SVector(x...))
...
sb1prob = ODEProblem{false}(
    sb1sys, [y[[1, 3]] .=> 1.0; y[[2, 4]] .=> 0.0], (0, 20.0), dt = 7.0e-3; u0_constructor = x -> SVector(x...)
)
...
sb2prob = ODEProblem{false}(sb2sys, y .=> 1.0, (0, 20.0), [α => 3], dt = 1.0e-2)
sb3prob = ODEProblem{false}(sb2sys, y .=> 1.0, (0, 20.0), [α => 8], dt = 1.0e-2)
sb4prob = ODEProblem{false}(sb2sys, y .=> 1.0, (0, 20.0), [α => 25], dt = 1.0e-2)
sb5prob = ODEProblem{false}(sb2sys, y .=> 1.0, (0, 20.0), [α => 100], dt = 1.0e-2)
...
sc1prob = ODEProblem{false}(sc1sys, y .=> 1.0, (0, 20.0), dt = 1.0e-2; u0_constructor = x -> SVector(x...))
...
sc2prob = ODEProblem{false}(sc2sys, y .=> 1.0, (0, 20.0), [β => 0.1], dt = 1.0e-2; u0_constructor = x -> SVector(x...))
sc3prob = ODEProblem{false}(sc2sys, y .=> 1.0, (0, 20.0), [β => 1.0], dt = 1.0e-2; u0_constructor = x -> SVector(x...))
sc4prob = ODEProblem{false}(sc2sys, y .=> 1.0, (0, 20.0), [β => 10.0], dt = 1.0e-2; u0_constructor = x -> SVector(x...))
sc5prob = ODEProblem{false}(sc2sys, y .=> 1.0, (0, 20.0), [β => 20.0], dt = 1.0e-2; u0_constructor = x -> SVector(x...))
...
sd2prob = ODEProblem{false}(
    sd2sys, [y[1] => 1.0; y[2:3] .=> 0.0], (0, 40.0), dt = 1.0e-5, cse = true; u0_constructor = x -> SVector(x...)
)

sd3sys = let
    sd3eqs = [
        D(y[1]) ~ y[3] - 100 * (y[1] * y[2]),
        D(y[2]) ~ y[3] + 2y[4] - 100 * (y[1] * y[2]) - 2.0e4 * y[2]^2,
        D(y[3]) ~ -y[3] + 100 * (y[1] * y[2]),
        D(y[4]) ~ -y[4] + 1.0e4 * y[2]^2,
    ]
...
sd5sys = let
    sd5eqs = [
        D(y[1]) ~ 0.01 - (1 + (y[1] + 1000) * (y[1] + 1)) * (0.01 + y[1] + y[2]),
...
sd5prob = ODEProblem{false}(sd5sys, y[1:2] .=> 0.0, (0, 100.0), dt = 1.0e-4, cse = true; u0_constructor = x -> SVector(x...))

sd6sys = let
    sd6eqs = [
        D(y[1]) ~ -y[1] + 10^8 * y[3] * (1 - y[1]),
        D(y[2]) ~ -10y[2] + 3.0e7 * y[3] * (1 - y[2]),
        D(y[3]) ~ -(-y[1] + 10^8 * y[3] * (1 - y[1])) - (-10y[2] + 3.0e7 * y[3] * (1 - y[2])),
    ]
...
se2prob = ODEProblem{false}(
    se2sys, [y[1] => 2.0, y[2] => 0.0], (0, 1.0), dt = 1.0e-3, cse = true; u0_constructor = x -> SVector(x...)
)

se3sys = let
    se3eqs = [
        D(y[1]) ~ -(55 + y[3]) * y[1] + 65 * y[2],
...
se4prob = ODEProblem{false}(
    se4sys, [y[1] => 0.0; y[2] => -2.0; y[3:4] .=> -1.0],
    (0, 1000.0), dt = 1.0e-3, cse = true; u0_constructor = x -> SVector(x...)
)
...
se5prob = ODEProblem{false}(
    se5sys, [y[1] => 1.76e-3; y[2:4] .=> 0.0], (0, 1000.0), dt = 1.0e-3, cse = true; u0_constructor = x -> SVector(x...)
)
...
sf2prob = ODEProblem{false}(
    sf2sys, [y[1] => 1.0; y[2] => 0.0], (0, 240.0), dt = 1.0e-2, cse = true; u0_constructor = x -> SVector(x...)
)

sf3sys = let
    sf3eqs = [
        D(y[1]) ~ -1.0e7 * y[2] * y[1] + 10 * y[3],
        D(y[2]) ~ -1.0e7 * y[2] * y[1] - 1.0e7 * y[2] * y[5] + 10 * y[3] + 10 * y[4],
        D(y[3]) ~ 1.0e7 * y[2] * y[1] - 1.001e4 * y[3] + 1.0e-3 * y[4],
        D(y[4]) ~ 1.0e4 * y[3] - 10.001 * y[4] + 1.0e7 * y[2] * y[5],
        D(y[5]) ~ 10 * y[4] - 1.0e7 * y[2] * y[5],
    ]
...
sf3prob = ODEProblem{false}(
    sf3sys, [y[1] => 4.0e-6; y[2] => 1.0e-6; y[3:5] .=> 0.0], (0, 100.0), dt = 1.0e-6, cse = true; u0_constructor = x -> SVector(x...)
)
...
sf4prob = ODEProblem{false}(
    sf4sys, [y[1] => 4.0; y[2] => 1.1; y[3] => 4.0], (0, 300.0), dt = 1.0e-3, cse = true; u0_constructor = x -> SVector(x...)
)

sf5sys = let
    sf5eqs = [
        D(y[1]) ~ 1.0e11 * (-3 * y[1] * y[2] + 0.0012 * y[4] - 9 * y[1] * y[3]),
        D(y[2]) ~ -3.0e11 * y[1] * y[2] + 2.0e7 * y[4],
        D(y[3]) ~ 1.0e11 * (-9 * y[1] * y[3] + 0.001 * y[4]),
        D(y[4]) ~ 1.0e11 * (3 * y[1] * y[2] - 0.0012 * y[4] + 9 * y[1] * y[3]),
    ]
...
sf5prob = ODEProblem{false}(
    ...
    (0, 100.0), dt = 1.0e-7, cse = true; u0_constructor = x -> SVector(x...)
)
...
ys = [
    3.42947415189, 3.35386959711, 1.35494901715,
    6.6414554255, 5.97156957878, 2.18231499728,
    11.2630437207, 14.6952576794, 6.27960525067,
    -30.1552268759, 1.65699966404, 1.43785752721,
    -21.123835338, 28.4465098142, 15.3882659679,
]
ys′ = [
    -0.557160570446, 0.505696783289, 0.230578543901,
    -0.415570776342, 0.365682722812, 0.169143213293,
    -0.325325669158, 0.189706021964, 0.087726532278,
    -0.024047625417, -0.287659532608, -0.117219543175,
    -0.176860753121, -0.216393453025, -0.014864789309,
]
y0 = y .=> reshape(ys, 3, 5)
...
y0 = [y[1] => 0.671396707141803; y[2] => 0.09540051444747446]
