...
# CUTEst.jl ships `classf.json`, the classification database behind
# `CUTEst.select_sif_problems`. It is generated by decoding every problem at its default
# size, so `variables.number` / `constraints.number` are the same `nvar` / `ncon` that
# `CUTEstModel(name)` reports, without paying for a SIF decode per candidate.
function classification_sizes()
    try
        path = joinpath(dirname(pathof(CUTEst)), "classf.json")
        data = CUTEst.JSON.parsefile(path)
        return Dict{String, Tuple{Int, Int}}(
            String(name) => (Int(entry["variables"]["number"]), Int(entry["constraints"]["number"]))
                for (name, entry) in data
        )
    catch err
        @warn "Unable to read the CUTEst classification database; falling back to decoding" exception = (
            err, catch_backtrace(),
        )
        return Dict{String, Tuple{Int, Int}}()
    end
...
function problem_size(name, sizes)
    entry = get(sizes, uppercase(name), nothing)
    if entry !== nothing && entry[1] >= 0 && entry[2] >= 0
        return (; ok = true, nvar = entry[1], ncon = entry[2])
    end

    return problem_metadata(name)
end
...
function select_safe_problems(
    ...
    for name in sort(unique(String.(candidates)))
        ...
        size = problem_size(name, sizes)

        size.ok || continue
        size.nvar <= max_var || continue
        size.ncon <= max_con || continue

        push!(buckets[nvar_bucket(size.nvar, edges)], name)
    end
