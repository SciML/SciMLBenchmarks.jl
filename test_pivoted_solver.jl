...
println("\n" * "="^70)
println("SELF-CONSISTENT SOLVER TEST")
println("="^70)
println()

try
    println("Solving with IDA (very relaxed tolerances)...")
    sol = solve(prob_dae, IDA(); abstol=1e-2, reltol=1e-2, dt=1e-3)
    ...
    res_final = zeros(NX)
    yp_final = zeros(NX)
    yp_final[1:7] = sol.u[end][8:14]
    yp_final[8:14] = sol.u[end][15:21]
    slider_crank_resmbs!(res_final, sol.u[end], yp_final, 0.1)
    
    println("  Final constraint residual: $(norm(res_final[22:24]))")
