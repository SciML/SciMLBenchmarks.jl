#!/usr/bin/env julia
# Test solve with corrected position AND consistent IC
...
function get_initial_conditions()
    ...
    y[1] = 0.0
    y[2] = 0.0
    y[3] = L1 + L2 + 0.169327969e-04  # CORRECTED POSITION
...
println("\n" * "="^70)
println("SOLVING WITH CORRECTED POSITION AND CONSISTENT IC")
println("="^70)
println("Initial conditions:")
println("  x₃(0) = $(y0[3]) (position constraint satisfied)")
println("  Residual @ t=0: ~ 1e-8 (acceptable)")

try
    println("\nAttempting solution with IDA (standard tolerances)...")
    sol = solve(prob_dae, IDA(); abstol=1e-6, reltol=1e-4)
    
    println("\n✓ SOLUTION SUCCESSFUL!")
    println("\nFinal state (t=$(sol.t[end])):")
    println("  x₃($(sol.t[end])) = $(sol.u[end][3])")
    println("  ω₁($(sol.t[end])) = $(sol.u[end][8])")
    println("  λ₂($(sol.t[end])) = $(sol.u[end][23])")
    println("\nNumber of steps: $(length(sol.t))")
    
catch e
    println("\n⚠ Standard tolerances failed, trying looser...")
    try
        sol = solve(prob_dae, IDA(); abstol=1e-4, reltol=1e-2)
        println("✓ SOLUTION SUCCESSFUL with loose tolerances!")
        println("\nFinal state (t=$(sol.t[end])):")
        println("  x₃($(sol.t[end])) = $(sol.u[end][3])")
        println("  ω₁($(sol.t[end])) = $(sol.u[end][8])")
        println("  λ₂($(sol.t[end])) = $(sol.u[end][23])")
