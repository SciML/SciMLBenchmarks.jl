...
println("\n" * "="^70)
println("SLIDER-CRANK INDEX-2 DAE TEST")
println("="^70)

try
    println("\nAttempting IDA solve with relaxed IC tolerance...")
    sol = solve(prob_dae, IDA(); abstol=1e-6, reltol=1e-4, maxiters=10000)
    println("✓ Solution computed successfully!")
    println("  Steps taken: $(length(sol))")
    println("  Position @ t=0.1: φ₁ = $(sol.u[end][1]), x₃ = $(sol.u[end][3])")
    println("  Elasticity @ t=0.1: q₄ = $(sol.u[end][7])")
catch e
    println("✗ Solve still fails")
    showerror(stderr, e)
end
